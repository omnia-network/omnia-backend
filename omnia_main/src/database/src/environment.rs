...
fn get_gateways_in_environment(environment_uid: EnvironmentUID) -> Vec<GatewayInfo> {
    let mut registered_gateways: Vec<GatewayInfo> = vec![];

    match get_environment_info_by_uid(&environment_uid) {
        Some(environment_info) => {
            for (uuid, info) in environment_info.env_gateways {
                ...
                registered_gateways.insert(0, gateway_info);
            }
        }
        None => print(format!(
...
fn get_devices_in_environment(environment_uid: EnvironmentUID) -> Vec<DeviceInfo> {
        ...
        Some(environment_info) => {
            for (gateway_uid, gateway_info) in environment_info.env_gateways {
                for (uuid, info) in gateway_info.devices {
                    ...
                    registered_devices.insert(0, device_info);
                }
            }
        }
        None => print(format!(
