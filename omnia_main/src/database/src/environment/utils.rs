use omnia_types::{
    device::{
        DeviceInfo, DeviceRegistrationInput, DeviceRegistrationResult, RegisteredDevicesInfo,
    },
    environment::{EnvironmentCreationInput, EnvironmentCreationResult, EnvironmentUID},
    gateway::{
        GatewayInfo, GatewayRegistrationInput, GatewayRegistrationResult, RegisteredGatewaysInfo,
    },
    user::PrincipalId,
};
...
fn create_new_environment(
    ...
    ENVIRONMENT_STORE.with(|environment_store| {
        environment_store.borrow_mut().insert(
            ...
            StoreTypes::StoredEnvironmentInfo {
    ...
    match get_environment_info_by_uid(&gateway_registration_input.env_uid) {
        Some(mut environment_info) => {
            ...
            ENVIRONMENT_STORE.with(|environment_store| {
                environment_store
                    .borrow_mut()
                    .insert(gateway_registration_input.env_uid, environment_info)
            });
    ...
    match get_environment_info_by_uid(&device_registration_input.env_uid) {
        Some(mut environment_info) => {
            match environment_info
                .env_gateways
                .remove(&device_registration_input.gateway_uid)
            {
                Some(mut gateway_info) => {
                    ...
                    ENVIRONMENT_STORE.with(|environment_store| {
                        environment_store
                            .borrow_mut()
                            .insert(device_registration_input.env_uid, environment_info)
                    });
...
fn get_gateways_in_environment(environment_uid: EnvironmentUID) -> Option<RegisteredGatewaysInfo> {
    let gateways = match get_environment_info_by_uid(&environment_uid) {
        Some(environment_info) => {
            ...
            for (uuid, info) in environment_info.env_gateways {
                ...
                registered_gateways_info
                    .registered_gateways
                    .insert(0, gateway_info);
            }
...
fn get_devices_in_environment(environment_uid: EnvironmentUID) -> Option<RegisteredDevicesInfo> {
        ...
        Some(environment_info) => {
            ...
            for (gateway_uid, gateway_info) in environment_info.env_gateways {
                for (uuid, info) in gateway_info.devices {
                    ...
                    registered_devices_info
                        .registered_devices
                        .insert(0, device_info);
                }
            }
...
pub fn get_environment_info_by_uid(
