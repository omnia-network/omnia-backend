use super::super::environment::utils as EnvironmentUtils;
...
fn set_user_in_environment(
    ...
    match get_user_profile_if_exists(user_principal) {
        Some(user_profile) => match EnvironmentUtils::get_environment_info_by_uid(&env_uid) {
            Some(environment_info) => {
                let updated_user_profile = ProfileStoreTypes::StoredUserProfile {
...
fn get_user_profile(user_principal_id: PrincipalId) -> ProfileStoreTypes::StoredUserProfile {
    let user_principal = Principal::from_text(user_principal_id).unwrap();
    ...
    match get_user_profile_if_exists(user_principal) {
        ...
        None => {
            ...
            let new_user_profile = ProfileStoreTypes::StoredUserProfile {
...
fn get_user_profile_if_exists(user_principal: Principal) -> Option<ProfileStoreTypes::StoredUserProfile> {
    USER_PROFILE_STORE.with(
        |profile_store| match profile_store.borrow().get(&user_principal) {
